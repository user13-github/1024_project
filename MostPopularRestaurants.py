#most popular restaurants using the provided tables:
...
# Perform aggregation
popular_restaurants_df = joined_df.groupBy("r.id", "r.name").agg(
    avg("r.score").alias("average_score"),
    count("r.ratings").alias("total_ratings")
)

# Order the results
ordered_restaurants_df = popular_restaurants_df.orderBy(
    popular_restaurants_df["average_score"].desc(), popular_restaurants_df["total_ratings"].desc()
)

# Show the results
ordered_restaurants_df.show(10)  # You can adjust the number of results to display
